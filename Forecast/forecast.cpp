...
#include "Pose/angle_solver.h"//                                                   zhytest
#include "Kalman/kalman3d.h"

#define DEBUG 1
#define max_time 60000
#define PICWIDTH 1280
#define PICHEIGHT 1024
#define SPEED 30
//60000
Mat show;
Mat k_show;
...
//    kalman_yaw.Kalman_init(KQ,KR);
//    kalman_xy.Kalman_init(KQ,KR);
//    kalman_yz.Kalman_init(KQ,KR);
//    Kalman_init(&KF);
...
inline double limit_time(double& time){
    int time_temp=time;
...
#ifdef DEBUG
 void drawForcast3d(Point3d before,Point3d after){
     cout<<"                                    xy:"<<after.x<<" "<<after.y<<endl;
     Rect rec1(before.x+PICWIDTH/2,before.y+PICHEIGHT/2,123,60);
     Rect rec2(after.x+PICWIDTH/2,after.y+PICHEIGHT/2,123,60);
     rectangle(show,rec1,Scalar(0,255,0));
     rectangle(show,rec2,Scalar(0,0,255));


//     circle(show,Point(before.x*resize+PICWIDTH/2,-before.y*resize+PICHEIGHT/2),50,Scalar(0,255,0),-1);
//     circle(show,Point(after.x*resize+PICWIDTH/2,-after.y*resize+PICHEIGHT/2),50,Scalar(0,0,254),-1);
//     namedWindow("Forcast",CV_WINDOW_NORMAL);
    imshow("Forcast",show);
//    waitKey(1);

 }
#endif
Point3d Forecast:: WorldForcast( Point3d original, double sys_time, double pitch_recv)
{
//    return original;
    //当前帧未发现目标
    if(original==Point3d(0,0,0))
    {
        lost_aim_num++;
        if(lost_aim_num >= lost_aim_max){
//            kalman_xy.Kalman_init(KQ,KR);
//            kalman_yz.Kalman_init(KQ,KR);
            record_history3d.clear();
            lost_aim_num=0;
        }
        return original;
    }
    //发现目标
    sys_time=limit_time(sys_time);

    result_center3d = KF3d.PredictByKF(original,sys_time,pitch_recv);
    result_center3d.y=original.y;

#ifdef DEBUG
    show=Mat(PICWIDTH,PICHEIGHT,CV_8UC3,Scalar(0,0,0));//观测结果，预测结果可视化
    k_show=Mat(PICWIDTH,PICHEIGHT,CV_8UC3,Scalar(0,0,0));
    drawForcast3d(original,result_center3d);
#endif
    return result_center3d;
 ...
 forcast(vector<Point2f> &original,double time,double y_p_recv[4])
 {
    ...
    k_show=Mat(1024,1080,CV_8UC3,Scalar(0,0,0));
     ...
     {   //当前帧未发现目标
         lost_aim_num++;
//         AngleSolver p_y
//        double p_y_err[0]=p_y_err[1]=0;
         if(lost_aim_num>=lost_aim_max)//超过max帧未发现目标则认为是丢失目标，清楚历史记录
         {
          record_history.clear();
          pitch_history.clear();
          lost_aim_num=lost_aim_num%50+lost_aim_max;
...
//          kalman_yaw.Kalman_init(KQ,KR);
     ...
     //***4.11更改，设置时间最大值record_history.clear();
     pitch_history.clear();
     lost_aim_num=lost_aim_num%50+lost_aim_max;
     //cout<<"                        lost_aim_num:"<<lost_aim_num<<endl;
     last_result={Point2f(0,0),Point2f(0,0),Point2f(0,0),Point2f(0,0)};
     //kalman_p.Kalman_init();
     //kalman_yaw.Kalman_init();
      int time_temp=time;
      ...
      if(recording_interval>=record_history_interval_max){
          ...
          pitch_history.emplace_back(y_p_recv[3]);
...
//        //result_center=kalman_p.Kalman_filter(result_center);
        double pitch_now_speed=y_p_recv[3];

        Point2f original_center=record_history[record_history.size()-1].center;
        if(pitch_history[pitch_history.size()-1]*pitch_history[pitch_history.size()-2]<0
                ||pitch_history[pitch_history.size()-2]*pitch_history[pitch_history.size()-3]<0
                ||pitch_history[pitch_history.size()-3]*pitch_history[pitch_history.size()-4]<0)
        {
            result_center=original_center;
        }
        else if(pitch_now_speed>0.2||pitch_now_speed<-0.2)
        {
//            result_center=kalman_p.Kalman_filter(record_history[record_history.size()-1].center,8,15);
            int add_yaw=100;

            if(pitch_now_speed>0)
            {
                cout<<"                         左"<<endl;
                result_center.x-=add_yaw;
            }
            else
            {
                cout<<"                         右"<<endl;
...
//       line(show,result_limited[3],result_limited[0],Scalar(255,255,0),1);
       ...
       {
           line(show,Point(i,0),Point(i,900),Scalar(100,100,100),2);
       }
...
double Forecast::my_gsl(data d, double aim_time)
{
 ...
 void Forecast::get_forecast()
  {/*虽然是二维图像，但是本质上x的变化和y的变化并无关联，应该视为两个独立变量，分别是关于时间t的函数*/
    ...
    {//分别为t,x,y,z历史值赋值
        t[i]=record_history3d[i].time;
        ft[0][i]=record_history3d[i].center.x;
        ft[1][i]=record_history3d[i].center.y;
        ft[2][i]=record_history3d[i].center.z;
    }
     //分别打包装入gsl算子
     d[0]={(size_t)record_history_size,t,ft[0]};
     d[1]={(size_t)record_history_size,t,ft[1]};
     d[2]={(size_t)record_history_size,t,ft[2]};


     //生成预测时间，此步应尽可能靠近计算时间，以减少不必要的误差
     double  aim_time;
     pre_time+=ft[2][record_history_size-1]/(SPEED*1000);
     getSystime(aim_time);
     //4.11更改，设置时间最大值
     ...
     result_center3d=Point3d(my_gsl(d[0],aim_time),my_gsl(d[1],temp_time+(pre_time>=0?0:pre_time)),my_gsl(d[2],aim_time));
}

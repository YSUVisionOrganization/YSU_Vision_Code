...
void ArmorDetector::InitArmor()
{
    ...
    // 初始化深度学习算法类
    this->yolov5_detector_ = new Yolov5("../model/model/opt-0625-001.xml", "../model/model/opt-0625-001.bin", 416, 416); // 创建yolov5detector对象
...
vector<Point2f>& ArmorDetector::DetectObjectArmor(){
    ...
    ClearAll();                    // 清除历史工作数据
    if(target_armor_point_set[0].y>1024)
    {
        cout<<"               detecy out of size:"<<target_armor_point_set[0]<<","<<target_armor_point_set[1]<<","<<target_armor_point_set[2]<<","<<target_armor_point_set[3]<<endl;
        target_armor_point_set[0]=Point2f(0,0);
        target_armor_point_set[1]=Point2f(0,0);
        target_armor_point_set[2]=Point2f(0,0);
        target_armor_point_set[3]=Point2f(0,0);
    }
